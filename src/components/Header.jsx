...
const Header = ({ bgStyle }) => {
	...
	const menuItems = [
		{ text: 'Курси', click: () => setVisibleBlock('packages'), hide: true },
		{
			text: 'Пройти тест',
			href: 'https://my.talken.cloud/student/registration/a92fca0b9fe1c4b12c8093aea33b9fdbe1577e3e',
			target: '_blank',
		},
		{ text: 'Оплата', href: '/pay', isNavLink: true },
		{ text: 'Кар’єра', href: '/career', isNavLink: true },
		{ text: 'Контакти', href: '/contacts', isNavLink: true },
		{ text: 'Корисне', href: '/blog', isNavLink: true },
	];

	return (
		<>
			<header
				className={`sm:mb-[63px] mb-[12px] ${
					bgStyle
						? 'adult_header py-[46px]'
						: 'pt-[42px] pb-3 lg:pb-0 lg:shadow-none shadow_item'
				}`}
			>
				<div className='max-w-full px-[20px] sm:max-w-[95%] xl:max-w-[90%] mx-auto flex items-center justify-between'>
					<NavLink to='/'>
						<img loading='lazy' src={logo} alt='StartSpeakEN' />
					</NavLink>
					<div className='lg:hidden flex items-center gap-5'>
						<a href='https://my.talken.cloud/login' className='user'>
							<FaRegUser />
						</a>
						<div
							className='grid justify-between items-center cursor-pointer h-[24px] gap-[2px]'
							onClick={() => setShowBurger(!showBurger)}
						>
							<span className='w-5 h-[3px] bg-[#232020]' />
							<span className='w-5 h-[3px] bg-[#232020]' />
							<span className='w-5 h-[3px] bg-[#232020]' />
						</div>
					</div>
					<nav className='lg:block hidden'>
						<ul className='flex items-center space-x-5 2xl:space-x-10 text-lg xl:text-xl font-medium'>
							{menuItems.map((item, index) => (
								<li
									key={index}
									style={
										item.hide && currentPath !== '/' && currentPath !== '/adult'
											? { display: 'none' }
											: {}
									}
									className='border-b-2 hover:border-[#232020] border-b-transparent duration-300'
								>
									{item.isNavLink ? (
					...
					<div className='lg:block hidden'>
						<Btn
							href='https://my.talken.cloud/login'
							target='_blank'
			...
			{showBurger && (
				<div className='overlay'>
					<div
						className={`bg-[#FEFCFB] pt-[58px] sm:w-[353px] w-[75%] pb-[28px] px-[25px] h-full  overflow-y-auto sm:h-screen transform duration-1000 ${
							showMob ? 'translate-x-0' : '-translate-x-full'
						}`}
					>
						<div className='flex items-start gap-[77px] mb-10'>
							<div>
								<img loading='lazy' src={logo} alt='StartSpeakEN' />
							</div>
							<svg
						...
						<ul className='flex flex-col items-start text-base sm:text-lg md:text-xl font-medium mb-5'>
							{menuItems.map((item, index) => (
								<li
									key={index}
									className='h-[40px] border-b border-b-[#D6D5D5] w-full flex items-center justify-start'
									onClick={handleCloseMenu}
									style={
										item.hide && currentPath !== '/' && currentPath !== '/adult'
											? { display: 'none' }
								...
								>
									{item.isNavLink ? (
										...
										<a
											href={item.href}
											onClick={
												item.click ? () => setVisibleBlock('packages') : null
											}
						...
						<a
							href='tel:+380678377966'
							className='text-base sm:text-lg md:text-xl font-medium mb-5'
						>
							+38 (067) 837-79-66
						</a>
						...
						<div className='flex flex-col gap-[25px] mt-10 items-start'>
							{forWho.map(item => (
								<NavLink
									to={item.to}
									key={item.text}
									onClick={handleCloseMenu}
									className='rounded-[14px] py-[2px] px-[2px] border-4 flex w-[217px] h-[61px] items-center'
									style={{ borderColor: item.color, background: item.color }}
								>
									<div
										className='flex items-center justify-center px-4 rounded-[14px]'
									...
									<div className='forWho_item w-full h-[53px]'>{item.text}</div>
								</NavLink>
							))}
						</div>
					</div>
